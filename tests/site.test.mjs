...
const readPage = (path) =>
  readFile(path === "/" ? "dist/index.html" : "dist" + path + ".html", "utf8");
test("every public route ships useful HTML and route-specific metadata before JavaScript", async () => {
  ...
  for (const path of routes) {
    ...
    assert.equal(
      ...
      1,
      path + " must have one primary heading",
    );
    assert.ok(
      html.includes('data-route="' + path + '"'),
      path + " must identify its prerendered route",
    );
    assert.ok(
      html.includes(
        'href="https://www.hyzl.ai' + (path === "/" ? "" : path) + '"',
      ),
      path + " canonical",
    );
  ...
  assert.equal(
    descriptions.size,
    routes.length,
    "descriptions must be specific to each route",
  );
});
